...
const pages = {
	...
	API_PAGE_ETHERNET: { num: 2, str: "/ethernet", title: "Ethernet" },
	API_PAGE_WIFI: { num: 3, str: "/wifi", title: "WiFi" },
	API_PAGE_SERIAL: { num: 4, str: "/zha-z2m", title: "ZHA/Z2M" },
	...
	API_PAGE_MQTT: { num: 8, str: "/mqtt", title: "MQTT Configuration" },
	API_PAGE_WG: { num: 9, str: "/wg", title: "WireGuard" }
}
...
function zbOta() {
	// Zigbee firmware update function - matches AZG project implementation
	// This function is called when user uploads a .hex file
	// The actual flashing is handled by the backend after file upload
	console.log("Zigbee OTA function called - file will be processed by backend");
}
...
function apiGetPage(page, doneCall) {
	...
	$("#pageContent").fadeOut(animDuration).load(apiLink + api.actions.API_GET_PAGE + "&page=" + page.num, function (response, status, xhr) {
		if (status == "error") {
			...
			$("title[data-replace='pageName']").text(page.title + " - AVATTO-GW90-Ti Zigbee Ethernet POE USB Adapter");//update page title
			...
			// 根据 operationalMode 动态切换设备运行模式图片
			if (values.operationalMode) {
				const mode = values.operationalMode.toLowerCase();
				// let imgSrc = "device_operating_lan.png";
				let imgSrc = "device_operating_default.png";
				if (mode.includes("wifi")) {
					// imgSrc = "device_operating_wifi.png";
                    let imgSrc = "device_operating_default.png";
				} else if (mode.includes("usb")) {
					// imgSrc = "device_operating_usb.png";
					let imgSrc = "device_operating_default.png";
				}
				// else default is LAN
...
function modalConstructor(type, params) {
	...
	switch (type) {
		...
		case "espFlashGitWait":
			...
			break;
		case "espGitUpdate":
			$(headerText).text("ESP32 update available").css("color", "#f7a623");
...
function updateProgress(targetValue) {
    ...
    const textEl = document.querySelector(".percent-num");

    // 1. 立即修改 CSS 变量，触发圆环和波浪的 CSS Transition 动画
    root.style.setProperty("--percent", targetValue);

    // 2. 根据进度值显示相应的文案而不是百分比
    const duration = 2000; // 动画持续时间，需与 CSS 中的 transition 时间对应
...
function getWifiList() {
	...
	updateProgress(0);
	// 禁用扫描按钮，防止重复点击
	$("#wifiScanBtn").prop("disabled", true);
	var timerId = setInterval(() => {
		// 在扫描过程中持续显示'扫描中'并循环动画
		updateProgress(50);
	}, 500);
...
async function processResponses() {
	try {
		let jsonUrl = `https://api.github.com/repos/${GITHUB_REPO_OWNER}/${GITHUB_REPO_NAME}/releases/latest`;
...
document.addEventListener("DOMContentLoaded", function () {
	...
	let preferredLang = savedLang || (languages.some((lang) => lang.value === browserLang) ? browserLang : "en");
...
// 本地化页面标题
function localizeTitle(pathname) {
	for (let key in pages) {
		if (pages[key].str === pathname) {
			const titleKey = pages[key].titleKey;
			if (titleKey) {
				document.title = i18next.t(titleKey);
				$("#pagenamePC").text(i18next.t(titleKey));
			}
			break;
		}
	}
